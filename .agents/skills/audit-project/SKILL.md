...
- **NUNCA executar auditorias sobre o meta-workspace ou a infraestrutura do sistema:** Esta habilidade destina-se exclusivamente a inspecionar projetos clientes em `projects/*` ou caminhos externos indicados pelo usuário.
- **NUNCA modificar arquivos ou executar comandos mutagênicos durante a auditoria:** O papel desta skill é diagnóstico e geração de relatório; correções requerem autorização explícita (HITL).
- **NUNCA registrar ou persistir credenciais/chaves encontradas em logs ou relatórios:** Qualquer segredo identificado deve ser mascarado imediatamente.
